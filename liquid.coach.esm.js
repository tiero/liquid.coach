...
var LiquidWallet = /*#__PURE__*/function () {
  ...
  _proto.sign = function sign(psbt, keyPair) {
    ...
    var inputIndex = psbt.data.inputs.findIndex(function (p) {
      return p.witnessUtxo.script.toString('hex') === wpkh.output.toString('hex');
    });
    psbt.signInput(inputIndex, keyPair);
    psbt.validateSignaturesOfInput(inputIndex); //Let's finalize all inputs

    psbt.validateSignaturesOfAllInputs();
    psbt.finalizeAllInputs();
    var hex = psbt.extractTransaction().toHex();
    return hex;
  };
